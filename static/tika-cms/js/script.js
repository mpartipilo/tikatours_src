var app = {
    ...
    init:function(p)
    {
        ...
        obj.makeSortable($('.row'));
        obj.makeSortable($(obj.getConfigItem('gridHolder')));
        obj.addColInRow();
    ...
    processTemplate:function(tmpl, data)
    {
        if($(tmpl).length)
        {
    ...
    buildGridContent:function()
    {
        if(typeof CKEDITOR !== undefined)
        {
            ...
            gridHolder = $(thisObj.getConfigItem('gridHolder'));

            if(triggerBtn.length && gridHolder.length )
            {
                triggerBtn.on('click', function(e){
                    e.preventDefault();

                    var  numColumns = parseInt($(thisObj.getConfigItem('numColumns')).val());
                    ...
                    if(numColumns)
                    {
                        ...
                        if(template.length)
                        {
                            ...
                            var processedTempl = thisObj.processTemplate(thisObj.getConfigItem('rowTemplate'), { rowIndex:rowInex, numColumns:numColumns, colIndex:''});
                            

                            gridHolder.append(processedTempl);
                            var newCols = $('#row-'+rowInex+' .editable-column-content textarea');
    ...
    makeSortable:function(elm)
    {
        if($(elm).length)
        {
            if($(elm).find('.sortable-item').length > 1)
            {
               $(elm).sortable({
