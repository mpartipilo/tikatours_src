...
function save_item()
{
	...
	$content_id = fetch_value("SELECT `id` FROM `content` WHERE `page_id` = '$id' AND `module_id` = '100' LIMIT 1");
	if(!$content_id)
	{
		$content_id = insert_row(array('page_id' => $id, 'module_id' => 100), 'content');
	}
    ...
    $message = "Sub category has been saved";
}
