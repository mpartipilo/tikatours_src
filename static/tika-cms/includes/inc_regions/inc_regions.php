...
function do_main(){
    ...
    $active_pages  = "";
    $page_contents = "";
    $sql           = "SELECT `id`, `name`, `rank`, `status`, `country_id`
                    FROM $tbl_name
                    WHERE `status` != 'D'
                    AND `country_id` != 0
                    ORDER BY `country_id`, `status`, `rank` ASC";

    $result = run_query($sql);
    while($row = mysql_fetch_assoc($result)) {
        extract($row);

        $country_name = fetch_value("SELECT `name` FROM `country` WHERE `id` = '$country_id' LIMIT 1");
        ...
        <table width="100%" class="bordered">
            <thead>
                <tr style="height:auto; font-weight:bold;">
                    ...
                    <th width="200" align="left">COUNTRY</th>
