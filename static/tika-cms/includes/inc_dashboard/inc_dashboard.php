...
function do_main(){
    ...
    $new_page = fetch_row("SELECT page_label, page_dateupdated FROM general_pages WHERE page_status != 'D' ORDER BY page_dateupdated DESC LIMIT 1");
    $old_page = fetch_row("SELECT page_label, page_dateupdated FROM general_pages WHERE page_status != 'D' ORDER BY page_dateupdated ASC LIMIT 1");

    $new_date = new DateTime($new_page['page_dateupdated']);
    $old_date = new DateTime($old_page['page_dateupdated']);
            ...
            <div class="dashboard-item">
                ...
                <div class="row">
                    <div class="col-xs-12 ditem-body">
                        ...
                        <p><strong>{$new_page['page_label']}:</strong> {$new_date->format('d F Y H:i:s')}</p>
                        <h2>Oldest page on website</h2>
                        <p><strong>{$old_page['page_label']}:</strong> {$old_date->format('d F Y H:i:s')}</p>
                    </div>
                </div>
            </div>
        </div>
