...
function display_login_screen()
{
    ...
    global $page_heading, $page_contents, $login_cls, $htmlroot;

    $ip_address = getenv('REMOTE_ADDR');

    // $is_blocked = fetch_value("SELECT `id` FROM `cms_blocked_user` WHERE `ip_address` = '{$ip_address}' AND `is_disabled` = 1");
    ...
    if(!$is_blocked)
    {
        ...
        $company      = fetch_value("SELECT `set_company` FROM `general_settings` WHERE `set_id` = '1' LIMIT 1");
        ...
        $redirect_to = "$htmlroot/tika-cms/index.php";
            ...
            <div class="col-xs-12 well well-small login-form{$login_cls}">
                ...
                <form method="post" action="{$redirect_to}" name="login" id="login">
                    ...
                    <fieldset>
                        ...
                        <div class="form-group">
                            <div style="margin:-15px 0 10px 0;"><img src="/captcha.jpg" alt="spam control image" id="anti-spam"></div>
                        </div>
                        <div class="form-group">
                ...
                <div>
                    <p id="licence-info">
                        <img src="/tika-cms/images/logo-circle.png" alt="Tomahawk" style="float:right;margin:-3px 0 0 10px;">
...
function add_user_to_blacklist($username, $is_valid_user = false)
{
    ...
    if($blocked_user)
    {
        ...
        if(($difference_hour >= $time_login_disabled) || !$is_disabled)
        {
            ...
            if($difference_hour >= $max_time_login_disabled)
            {
                ...
                run_query("UPDATE `cms_blocked_user` SET `failed_login_attempt_count` = 1, `total_failed_attempt` = (`total_failed_attempt` + 1), 
            ...
            else
            {
                ...
                if($failed_login_attempt_count < $max_login_attempts)
                {
                    ...
                    run_query("UPDATE `cms_blocked_user` SET `failed_login_attempt_count` = (`failed_login_attempt_count` + 1), 
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        {
                            run_query("UPDATE `cms_blocked_user` SET `total_failed_attempt` = (`total_failed_attempt` + 1), 
    ...
    else
    {
        ...
        insert_row($data_arr, 'cms_blocked_user');
...
function remove_user_from_blacklist($username)
{
    ...
    $sql = "DELETE FROM `cms_blocked_user` WHERE `username` = '{$username}' LIMIT 1";
        ...
        if($blocked_user['is_disabled'])
        {
            ...
            if($failed_hour_count >= $max_disabled_hours)
            {
                if(!$is_notified)
                {
                    ...
                    {
                        run_query("UPDATE `cms_blocked_user` SET `is_notified` = 1 WHERE `id` = '{$id}' LIMIT 1");
                    }
                }
            ...
            else
            {
                ...
                if($difference_hour > $time_login_disabled) 
                {
                    run_query("UPDATE `cms_blocked_user` SET `failed_login_attempt_count` = 0, `recent_login_attempt_on` = NOW(),
...
function do_login()
{
   ...
   global $message, $valid , $locked, $rootadmin, $rootfull;
    ...
    if($email && $password && strlen($password) > 5 && $captcha_is_valid && !$user_is_locked)
    {
        ...
        if(!empty($user_details))
        {
            ...
            if($force_do || ($force_action && $force_id))
            {
                $redirect_to = "$htmlroot/$foldername/index.php";
