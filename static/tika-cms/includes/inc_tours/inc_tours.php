...
function do_main(){
    ...
    $active_pages  = "";
    $page_contents = "";
    $sql           = "SELECT `id`, `name`, `rank`, `status`,`main_category_id`,`sub_category_id`,`is_featured`
                    FROM $tbl_name
                    WHERE `status` != 'D'
    ...
    while($row = mysql_fetch_assoc($result)) {
        ...
        $cat_name     = fetch_value("SELECT `name` FROM `tour_category` WHERE `id` = '$main_category_id' LIMIT 1");
        $sub_cat_name = fetch_value("SELECT `name` FROM `tour_category` WHERE `id` = '$sub_category_id' LIMIT 1");
        ...
        if ($is_featured == 1)
        {
            $feat = '<span class="label label-danger">Featured</span>';
        }
        ...
        $active_pages .= <<< HTML
            ...
            <tr>
                ...
                <td width="150">$cat_name</td>
                <td width="150">$sub_cat_name</td>
                <td width="100">$feat</td>
        ...
        <table width="100%" class="bordered">
            <thead>
                <tr style="height:auto; font-weight:bold;">
                    ...
                    <th width="450" align="left">NAME</th>
                    <th width="150" align="left">MAIN CATEGORY</th>
                    <th width="150" align="left">SUB CATEGORY</th>
                    <th width="100" align="left">FEATURED</th>
