...
class images extends filenames{
    ...
    public $maxwidth, $maxheight, $path, $image, $savetype, $error_arr, $orientation_relative,
            $orientation, $newheight, $newwidth, $suffix, $savedirectory, $newimage;
    ...
    function saveImage($crop=false){
        ...
        switch(strtolower($this->savetype)){
            case('jpg'): case('jpeg'):
                imagejpeg($this->newimage,$this->savedirectory.'/'.$this->filename.$this->suffix.'.'.$this->savetype,$this->quality);
                break;
            case('gif'):
                imagegif($this->newimage,$this->savedirectory.'/'.$this->filename.$this->suffix.'.'.$this->savetype,$this->quality);
                break;
            case('png'):
                imagepng($this->newimage,$this->savedirectory.'/'.$this->filename.$this->suffix.'.'.$this->savetype,$this->quality);
                break;
            //case('wbmp'): case('bmp'):
    ...
    function resizer($saveDir, $path, $maxWidth, $maxHeight,$saveType='jpg', $scaleType='fill'){
        $this->setSaveDirectory($saveDir);
        $this->path = $path;
        $this->setMaxWidth($maxWidth);
        $this->setMaxHeight($maxHeight);
        if($saveType === 'jpg' || $saveType === 'jpeg') $this->setQuality(80);
