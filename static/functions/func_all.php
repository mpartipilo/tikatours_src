...
function getAbsoluteParentId($cursorpage_id){
	global $absparent;
	$sql = "SELECT page_id, page_parentid
                FROM general_pages
                WHERE page_id = $cursorpage_id";
	$parent_arr = fetch_row($sql);
	$cursorpage_parent 	= $parent_arr['page_parentid'];
	$cursorpage_id 		= $parent_arr['page_id'];
...
if(!function_exists('page_list'))
{
	...
	function page_list($is_mobile= false, $parent=0, $sel='', $sel_field='page_parentid',  $fields=array('page_id', 'page_label', 'page_parentid'))
    {
    	...
    	WHERE page_status = 'A'";
    	$sql .= ($is_mobile) ? " AND page_mobile ='M'" : " AND (page_mobile IS NULL OR page_mobile = '')";
    	...
    	if(count($pages) > 0)
    	{
    		...
    		foreach ($pages as $page)
    		{
    			...
    			$label = $hellip.(($page['page_label']) ? $page['page_label'] : '');
		    	...
		    	$html .= page_list($is_mobile, $page_id, $sel, $sel_field, $fields);
